...
void GetGB2312Code ( uint8_t * pBuffer, uint16_t code)
{
	...
	fontOffset = ((GB2312_H - 0xa1)*94 + GB2312_L - 0xa1) * LCD_CHFonts->Sizes	;						// 计算当前汉字的偏移地址
	QSPI_W25Qxx_ReadBuffer(pBuffer,LCD_CHFonts->FontAddress + fontOffset,LCD_CHFonts->Sizes);		// 从SPI Flash读取数据
...
void  LCD_DisplayNumber( uint16_t x, uint16_t y, int32_t number, uint8_t len)
{
	...
	{
		sprintf( Number_Buffer , "%ld", number );	// 将 number 转换成字符串，便于显示
	}
	else			// 多余位补空格
	{
		sprintf( Number_Buffer , "%ld", number );	// 将 number 转换成字符串，便于显示
